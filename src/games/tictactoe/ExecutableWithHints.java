...
/**
 * La classe ExecutableWithHints contient la méthode principale pour exécuter une partie de Tic Tac Toe avec ou sans indices.
 * Elle gère l'interaction avec les utilisateurs, y compris la saisie des noms des joueurs et des coups.
 */
class ExecutableWithHints{
     ...
     * Cette méthode permet également d'afficher les menaces potentielles de l'adversaire 
     * pendant la partie si l'option des indices est activée.
     *
    ...
    static void startGame(String j1, String j2) {
		...
		// Demande si le joueur souhaite jouer avec des indices 
		System.out.println("Est-ce que vous voulez jouer avec des indices?");
		String reponse = scanner.next();
		
		TicTacToe jeux;
		
		if (reponse.equals("oui")) {
			jeux = new TicTacToeWithHints(j1, j2);
		} 
		else {
			jeux = new TicTacToe(j1, j2);
		}
